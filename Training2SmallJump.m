...
movedist = 50; %Distance dot moves
rewardmin = .18; %Reward duration range min
rewardmax = .18; %Reward duration range max
...
height = 100; %Height of window for reward
width = 100; %Width of window for reward
c=clock;
elapsedtimestart = (c(5));
eye = 1;
        ...
        if(between == 0 && feedback == 0)
            ...
            elseif((fixating == 1) && (GetSecs > (fixmin + fixtime))) %
                if(moved)
                    ...
                    disp(['Fixation Dot #' num2str(trial)]);
                    c1=clock;
                    elapsedtimetotal = (roundn((c1(5)-elapsedtimestart),-2));
                    disp(['Elapsed Time:' num2str(elapsedtimetotal)]);
                    disp(['Rewardtime: ' num2str(rewardduration)]);
                    targX = (rand() * 624) + 200;
                    targY = (rand() * 468) + 150;
                    moved = 0;
                    fixating = 0;
